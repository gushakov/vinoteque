...
/**
 * Manages all queries to the local HSQL database. For configuration see
...
public class HsqldbDao {
    ...
    public List<Entry> getEntries(Column column) {
        return jdbcTemplate.getJdbcOperations().query("select * from public.entries where column = ? order by lcase(ltrim(name))",
                ParameterizedBeanPropertyRowMapper.newInstance(Entry.class),
                column.name());
    }

    public List<Vin> getSomeVins(int limit) {
        return jdbcTemplate.getJdbcOperations().query("select * from public.vins limit ?",
    ...
    public List<Entry> getDistinct(Column column) {
        String sql = "select distinct "
                ...
                + column.name()
                + ")) asc";
        return jdbcTemplate.getJdbcOperations().query(sql, ParameterizedBeanPropertyRowMapper.newInstance(Entry.class));
    }
    ...
    public boolean addEntry(Entry entry, boolean withCheck) {
        ...
        if (withCheck) {
            ...
            int count = jdbcTemplate.getJdbcOperations().queryForInt(existsSql, entry.getName(), entry.getColumn());
            if (count == 0) {
    ...
    /**
     * Executes
     * <code>ALTER TABLE</code> statement to add new column. If
     * <code>defaultValue</code> is specified, then executes
     * <code>UPDATE</code> statement to set all values in the column to the
    ...
    /**
     * Deletes all vins from the
     * <code>VINS</code> and reinserts them back grouped by value of
     * <code>CASIER</code> column
     */
    public void groupByCasier() {
        //get all vins
        List<Vin> vins = getAllVins();
        //sort by column
        Collections.sort(vins, new Comparator<Vin>() {
            @Override
            public int compare(Vin v1, Vin v2) {
                return new Integer(v1.getCasier()).compareTo(new Integer(v2.getCasier()));
            }
        });
        //truncate the table
        jdbcTemplate.getJdbcOperations().execute("truncate table public.vins");
        //insert the sorted vins
        addVins(vins);
    }
}
